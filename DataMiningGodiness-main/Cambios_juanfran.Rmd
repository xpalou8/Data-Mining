...
##Focusing on US data
```{r}
US_data = data_no_outlier[ data_no_outlier$company_location == "US",]
#removing columns from US_data
US_data["company_location"] <- NULL
head(nrow(US_data))
head(US_data)
summary(lm(formula = salary_in_usd ~ work_year + experience_level + job_title + remote_ratio + company_size, 
           data = US_data))
```
